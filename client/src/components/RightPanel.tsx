...
export default function RightPanel({
  ...
  return (
    <Tabs
      ...
      defaultValue="events"
      className="h-full flex flex-col gap-0 glass-panel-solid border-0"
    >
      {/* 标签栏 */}
      <TabsList className="w-full rounded-none border-b border-white/[0.06] bg-transparent h-9 p-0 shrink-0">
        <TabsTrigger
          ...
          value="events"
          className="flex-1 rounded-none text-xs gap-1.5 data-[state=active]:bg-white/[0.04] data-[state=active]:shadow-none border-b-2 border-transparent data-[state=active]:border-primary"
        >
  ...
  return (
    <div className="p-2 space-y-1.5">
      ...
      {events.map((ev, i) => (
        ...
        >
          <div className="flex items-center gap-1.5 mb-1">
            <span className="text-xs font-medium" style={{ color: ev.color }}>
              {ev.botName}
            </span>
            {ev.time && (
              <span className="text-[10px] text-muted-foreground/40">
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              >
                <PixelAvatar color={color} size={14} />
              </div>
              <div>
              ...
              <div className="mt-2 p-2 rounded-md space-y-1 bg-white/[0.02] border border-white/[0.06]">
                {moment.comments.slice(-2).map((c, i) => (
                  <div key={i} className="text-xs">
                    <span style={{ color: BOT_COLORS[c.bot_id] || '#4d96ff' }}>
                      {c.bot_name}:{' '}
                    </span>
      ...
      <div className="flex flex-wrap gap-1 mb-3">
        {LOC_NAMES.map(loc => {
          ...
          return (
            ...
            >
              {cfg.icon} {loc.slice(0, 4)}{' '}
              {count > 0 && (
                <span className="text-red-400 ml-0.5">·{count}</span>
              )}
            </Badge>
          )
        })}
...
function LocationDetail({
  ...
  return (
    <div className="space-y-3">
      {/* 地点头部 */}
      <div
        className="p-2.5 rounded-md glass-card"
        style={{ borderColor: `${cfg?.color || '#4d96ff'}25` }}
      >
        ...
        <div>
          ...
          <div className="space-y-1">
            {data.public_memory.slice(-3).map((mem, i) => (
              <div
                key={i}
                className="text-xs p-2 rounded-md glass-card text-foreground/55"
              >
