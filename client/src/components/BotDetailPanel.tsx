...
export default function BotDetailPanel({
  ...
  return (
    <div
      data-testid="bot-detail-content"
      className="h-full flex flex-col overflow-hidden animate-slide-right glass-panel-solid"
    >
      {/* 头部 */}
      <div className="flex items-center gap-3 px-3 py-2.5 shrink-0 border-b border-white/[0.08]">
        ...
        >
          <PixelAvatar color={color} size={22} />
        </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={label} className="flex items-center gap-1.5">
                  ...
                  />
                  <span
                    className="text-xs w-6 text-right font-mono-data"
                    style={{ color: c }}
                  >
                    {Math.round(value)}
                  </span>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(bot.skills).map(([skill, val]) => (
                    <div key={skill} className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs w-5 text-right font-mono-data text-muted-foreground">
                        {val}
                      </span>
                    </div>
                  ))}
                ...
                <div className="space-y-1.5">
                  {[...bot.action_log]
                    ...
                    .map((log, i) => (
                      <div
                        key={i}
                        className="text-xs p-2 rounded-md glass-card animate-fade-in-up"
                      >
                        ...
                        {log.result?.narrative && (
                          <div className="text-foreground/60">
                            {log.result.narrative.slice(0, 60)}
                          </div>
                        )}
                      </div>
                    ))}
